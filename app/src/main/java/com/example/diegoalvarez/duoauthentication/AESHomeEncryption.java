...
public class AESHomeEncryption extends AppCompatActivity {
    ...
    private char[] left_rotate(char[] arr, int times) {
        assert (arr.length == 4);
        if (times % 4 == 0) {
            return arr;
        }
    ...
    public void MixColumns(char[][] arr) //method for mixColumns
    {
    ...
    private char mcHelper(char[][] arr, int[][] g, int i, int j)
    {
        char mcsum = 0;
        for (int k = 0; k < 4; k++) {
            int a = g[i][k];
    ...
    //Decryption shift
    public void inverseShiftRows(char[][] state) {
        for (int i = 1; i < 4; i++) {
    ...
    private char[] right_rotate(char[] arr, int times) {
    ...
    public void inverseSubBytes(char[][] state) {
        for (int i = 0; i < 4; i++) {
            for (int j = 0; j < 4; j++) {
    ...
    public void inverseMixColumns(char[][] arr) {
        char[][] tarr = new char[4][4];
        for(int i = 0; i < 4; i++)
        {
            System.arraycopy(arr[i], 0, tarr[i], 0, 4);
    ...
    private char invMcHelper(char[][] arr, int[][] igalois, int i, int j) //Helper method for invMixColumns
    {
        char mcsum = 0;
        for (int k = 0; k < 4; k++) {
            int a = igalois[i][k];
    ...
    public void xOrRconVal(char[] temp, int iteration) {
        //rotate temp
        char one = temp[0];
        for (int i = 0; i < 3; i++) {
            temp[i] = temp[i + 1];
    ...
    public char[] keyExpansion(char[] key) {
        int n = BLOCK_SIZE / 4;
     ...
     */
    public byte[] AES_Encrypt(String message, char[] key) {
